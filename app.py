...
st.set_page_config(
    page_title="SoLEXS Solar Flare Dashboard",
    page_icon="☀️",
    layout="wide",
...
# -----------------------------------------------------------------------------
# Data Loading & Caching Functions (Hyper-Optimized)
# -----------------------------------------------------------------------------
@st.cache_data
...
# -----------------------------------------------------------------------------
st.markdown('<div class="main-title">☀️ SoLEXS (Aditya-L1) Solar Flare Dashboard</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">Full Interactive X-ray Light Curve Analysis, Spectral Diagnostics, Gap Quality & Flare Risk Forecasting</div>', unsafe_allow_html=True)
...
if df_ts is None:
    st.error("⚠️ Primary dataset `solexs_master_timeseries.parquet` not found! Please run `batch_load.py` first.")
    st.stop()
...
if section == "1. Overview":
    st.header("📊 Executive Overview & Instrumentation Metrics")
    ...
    col2.metric("Total Date Range", f"{n_dates} Days")
    ...
    st.markdown("---")
    st.subheader("Instrument & Pipeline Characteristics")
    ...
    with col_a:
        ...
        **Satellite & Detector Specifications:**
        ...
        - **Observation Date Range**: `{min_d}` to `{max_d}`
        - **Energy Spectrum**: 340 Energy Channels (1.0 keV – 30.0 keV)
        - **Sampling Frequency**: 1.0 Hz (1-second time resolution)
        """)
    with col_b:
        st.markdown(f"""
        **Pipeline Data Quality Record:**
        - **All-NaN Rows Dropped**: `245,917` rows
        - **Valid Measured Spectra**: `{total_rows:,}` rows
        - **Mean Measured Count Rate**: `{df_ts['total_counts'].mean():.2f}` counts/s
        - **Peak Measured Count Rate**: `{df_ts['total_counts'].max():,.0f}` counts/s
        """)
        
    st.markdown("### Selected Window Timeseries Data Sample")
...
# -----------------------------------------------------------------------------
    ...
    st.header("📈 Interactive Multi-Day Light Curve")
    ...
    y_mode = col_ctrl3.radio("Y-Axis Mode", ["Raw total_counts", "Background-subtracted excess"])
    
    y_col = 'total_counts' if y_mode == "Raw total_counts" else 'excess'
    
    # Strict max 3,000 points downsampling for instant browser Plotly rendering
...
# -----------------------------------------------------------------------------
    ...
    st.header("🖼️ Small-Multiples Per-Day Light Curve Grid")
    ...
    if num_dates == 0:
        ...
        st.write(f"Displaying {num_dates} calendar date subplots (5-column grid):")
        cols = 5
        ...
        for idx, date_str in enumerate(present_dates):
            ...
            # Downsample day points to 200 points max for instant rendering
...
# -----------------------------------------------------------------------------
    ...
    st.header("🔍 Flare Event Candidate Catalog Explorer")
    ...
    if cat_df is None or cat_df.empty:
        ...
        st.markdown("---")
        st.subheader("Select Event for Diagnostic Analysis")
        ...
        if not event_list:
            ...
            with col_zoom:
                ...
                fig_zoom = px.line(sub_zoom, x='utc_time', y='total_counts', title=f"Event {selected_event_id} Light Curve")
            ...
            with col_spec:
                ...
                if counts_2d is not None and pk_index < len(counts_2d):
                    ...
                    fig_spec.update_layout(xaxis_title="Energy Channel (0-339)", yaxis_title="Counts", template="plotly_white")
...
# -----------------------------------------------------------------------------
    ...
    st.header("🔥 Time-vs-Channel Energy Spectrogram Heatmap")
    ...
    if counts_2d is None:
        ...
        if len(sub_indices) == 0:
            ...
            # Downsample to max 400 columns for sub-second Plotly rendering
            ...
            fig_spec = px.imshow(
                ...
                labels=dict(x="UTC Time", y="Energy Channel (0-339)", color="log10(Counts)"),
                x=sample_times,
                y=np.arange(340),
                aspect="auto",
...
# -----------------------------------------------------------------------------
    ...
    st.header("📊 Daily Summary Statistics & Coverage Flags")
...
# -----------------------------------------------------------------------------
    ...
    st.header("⚠️ Timeline Gap & Data Quality Breakdown")
...
# -----------------------------------------------------------------------------
    ...
    st.header("🔮 XGBoost Flare Forecasting & Risk Timeline")
    
    if pred_df is None or model_meta is None:
        st.warning("⚠️ Trained model predictions summary or metadata JSON missing! Run `train_model.py` first.")
    else:
        ...
        with col_ms:
            st.markdown("**Misses (Actual Imminent Flare with $P \\le 0.5$ Alert Risk):**")
...
# -----------------------------------------------------------------------------
    ...
    st.header("🛰️ NOAA/GOES Ground-Truth Flare Cross-Match Table")
    
    st.markdown("""
    **Cross-Match Verification**:
    The following table details real NOAA/GOES satellite-reported solar flares cross-matched with detected SoLEXS peak candidate events within a $\\pm 5$ minute window:
    """)
